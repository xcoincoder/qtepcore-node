...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'qtepcore-lib': 'https://github.com/xcoincoder/qtepcore-lib.git#master',
    'qtepcore-node': 'https://github.com/xcoincoder/qtepcore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/qtepcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the qtepcore-node directory and configuration
