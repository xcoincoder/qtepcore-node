...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for qtepd is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and qtepcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed qtepcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './qtepcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in qtepcore-node services directory
    ...
    if (servicePackage.qtepcoreNode) {
      serviceModule = service.name + '/' + servicePackage.qtepcoreNode;
    }
