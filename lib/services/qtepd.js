...
/**
 * Provides a friendly event driven API to qtepd in Node.js. Manages starting and
 * stopping qtepd as a child process for application support, as well as connecting
 * to multiple qtepd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Qtep(options) {
  if (!(this instanceof Qtep)) {
    return new Qtep(options);
  }
...
util.inherits(Qtep, Service);

Qtep.dependencies = [];

Qtep.DEFAULT_MAX_TXIDS = 1000;
Qtep.DEFAULT_MAX_HISTORY = 50;
Qtep.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Qtep.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Qtep.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Qtep.DEFAULT_SPAWN_RESTART_TIME = 5000;
Qtep.DEFAULT_SPAWN_STOP_TIME = 10000;
Qtep.DEFAULT_TRY_ALL_INTERVAL = 1000;
Qtep.DEFAULT_REINDEX_INTERVAL = 10000;
Qtep.DEFAULT_START_RETRY_INTERVAL = 5000;
Qtep.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Qtep.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Qtep.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Qtep.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Qtep.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Qtep.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Qtep.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Qtep.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Qtep.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Qtep.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Qtep.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Qtep.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Qtep.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Qtep.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Qtep.prototype._initCaches = function() {
...
Qtep.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Qtep.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Qtep.prototype.getPublishEvents = function() {
  return [
    {
...
Qtep.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'qtepd/' + name, 'total:', this.subscriptions[name].length);
};

Qtep.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'qtepd/' + name, 'total:', this.subscriptions[name].length);
};


Qtep.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
Qtep.prototype.unsubscribeBalance = function(emitter, addresses) {
...
Qtep.prototype.unsubscribeBalanceAll = function(emitter) {
...
Qtep.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'qtepd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Qtep.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'qtepd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Qtep.prototype.unsubscribeAddressAll = function(emitter) {
...
Qtep.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Qtep.DEFAULT_CONFIG_SETTINGS;
...
Qtep.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Qtep.prototype._expandRelativeDatadir = function() {
...
Qtep.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in qtepd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in qtepd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in qtepd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './qtep.conf');

  log.info('Using qtep config file:', configPath);
...
Qtep.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of qtepcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of qtepcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of qtepcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to qtepd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from qtepd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from qtepd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that qtepd is undergoing a reindex. ' +
             ...
             'of qtepcore-node services will start.');
    node._reindex = true;
  }
};

Qtep.prototype._resetCaches = function() {
...
Qtep.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Qtep.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Qtep.prototype._initChain = function(callback) {
  var self = this;
...
Qtep.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
Qtep.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/qtep.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/qtep.conf';
    }
  }
  return networkPath;
};

Qtep.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Qtep.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('qtepd/hashblock', message.toString('hex'));
    }
  }

};

Qtep.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Qtep.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Qtep Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Qtep.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Qtep.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Qtep.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
Qtep.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('qtepd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Qtep.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('qtepd/rawtransaction', message.toString('hex'));
    }
...
Qtep.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Qtep.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Qtep.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Qtep.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Qtep.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Qtep Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Qtep.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Qtep.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Qtep.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/qtepd.pid';
...
Qtep.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting qtep process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Qtep process unexpectedly exited with code:', code);
        log.warn('Restarting qtep child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Qtep process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn qtepd.'));
      }
...
Qtep.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to qtepd.'));
    }
 ...
 */
Qtep.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Qtep configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Qtep.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Qtep.prototype.syncPercentage = function(callback) {
  var self = this;
...
Qtep.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, qtepcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Qtep.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Qtep.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Qtep.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Qtep.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in qtepcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Qtep.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Qtep.prototype._getConfirmationsDetail = function(transaction) {
...
Qtep.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Qtep.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Qtep.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A qtepcoin transaction id
 * @param {Function} callback
 */
Qtep.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Qtep.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Qtep.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Qtep.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Qtep.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Qtep.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Qtep.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Qtep.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Qtep.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Qtep.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
Qtep.prototype.estimateSmartFee = function (blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Qtepcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Qtep.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.getInfo = function(callback) {
...
Qtep.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('qtepd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('qtepd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Qtep.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Qtep.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Qtep.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Qtep.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Qtep.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Qtep.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Qtep.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Qtep.prototype.getStakingInfo = function(callback) {
    var self = this,
